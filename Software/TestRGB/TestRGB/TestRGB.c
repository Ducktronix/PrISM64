...
const char numbers[] = {
0x00,0x80,0x80,0xFF,0x82,0x84,0x00,0x00,  //0x00,0x00,0x84,0x82,0xFF,0x80,0x80,0x00, //49/ --> 1  flipped the order and it flipped the image
...
void parse_frame(char * postBuffer)
{
    ...
    for(int LED = 0 ; LED < 64 ; LED++)
    {
        ...
        green_frame[LED] = (color_value & 0xE0) >> 5;	//(temp & 0b11100000) >> 5; Highest 3 bits represent the Green value for the current LED
        red_frame[LED] = (color_value & 0x1C) >> 2; 	//(temp & 0b00011100) >> 2; Next 3 bits represent the Red value for the current LED
